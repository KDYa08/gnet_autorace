...
class ConstructionSign(LifecycleNode):
    def __init__(self):
        super().__init__('construction_sign')
    ...
    def on_configure(self, state: State):
        ...
        if self.sim:
            self.CompressedImage = self.create_subscription(CompressedImage, '/camera/image_raw/compressed', self.construction_sign_detect, self.qos_profile)
            self.get_logger().info("sim node mode")
        else:
            self.CompressedImage = self.create_subscription(CompressedImage, '/csi_camera1/compressed', self.construction_sign_detect, self.qos_profile)
    ...
    def construction_sign_detect(self, img):
        if self.activate == True:
            ...
            try:
                ...
                for box in zip(*box_loc[::-1]):
                    ...
                    self.get_logger().info("construction sign check!")
                    msg.data = 'construction_sign'
                    self.activate = False
                    cv2.destroyAllWindows()
            ...
            try:
                ...
                cv2.imshow('construction_sign', self.traffic_img)
            except Exception as e:
                pass
                #self.get_logger().info(f"{e}")
