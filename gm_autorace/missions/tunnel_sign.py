...
class TunnelSign(LifecycleNode):
    def __init__(self):
        super().__init__('tunnel_sign')
    ...
    def on_configure(self, state: State):
        ...
        if self.sim:
            self.CompressedImage = self.create_subscription(CompressedImage, '/camera/image_raw/compressed', self.tunnel_sign_detect, self.qos_profile)
            self.get_logger().info("sim node mode")
        else:
            self.CompressedImage = self.create_subscription(CompressedImage, '/csi_camera1/compressed', self.tunnel_sign_detect, self.qos_profile)
    ...
    def tunnel_sign_detect(self, img):
        if self.activate == True:
            ...
            try:
                ...
                for box in zip(*box_loc[::-1]):
                    ...
                    self.get_logger().info("tunnel sign check!")
            ...
            try:
                ...
                cv2.imshow('tunnel_sign', self.traffic_img)
            except Exception as e:
                self.get_logger().info(f"{e}")
                pass
